...
```{r, echo = FALSE}
## read in raw administrative data
df <- readr::read_csv('../tests/testthat/testdata/admin_data.csv',
...
deid_dua(df, write_crosswalk = TRUE, id_length = 20,
         crosswalk_name = file.path(tmpdir, 'tmp'))
```
```{r, eval = FALSE}
## deidentify data
deid_dua(df, write_crosswalk = TRUE, id_length = 20)
...
```{r, echo = FALSE}
## show crosswalk
cw <- readr::read_csv(file.path(tmpdir, 'tmp.csv'), col_types = cols(.default = 'c'))
cw
rm(tmpdir)
```
...
```{r, echo = FALSE}
deid_dua(df, existing_crosswalk = file.path(tmpdir, 'crosswalk_full.csv'))
rm(tmpdir)
```
...
```{r, echo = FALSE}
deid_dua(df, existing_crosswalk = file.path(tmpdir, 'crosswalk_partial.csv'))
