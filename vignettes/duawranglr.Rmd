...
```
```{r, echo = FALSE}
set_dua('../tests/testthat/testdata/dua_one.csv')
```
## Check DUA options
...
## deidentify data
tmpdir <- tempdir()
deidentify(df, write_crosswalk = TRUE, id_length = 20,
...
dua_write(df, 'cleaned_data.csv', output_type = 'csv')
```
```{r}
## check
check_protect(df)
...
```{r, eval = FALSE}
make_template('clean_data.R')
```
```{r, echo = FALSE}
writeLines(readLines('../tests/testthat/testdata/template_example.R'))
```
